...
class ScodobjventascaractController extends Controller
{
	/**
	...
	{
		$model=new Scodobjventascaract;

		// obtener el file path del archivo
		$sPathFile = Yii::getPathOfAlias('webroot').'/archivos/';
	...
	{
		$dataProvider=new CActiveDataProvider('Scodobjventascaract');
	...
	{
		$model=new Scodobjventascaract('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Scodobjventascaract']))
			$model->attributes=$_GET['Scodobjventascaract'];
	...
	{
		$model=Scodobjventascaract::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='scodobjventascaract-form')
		{
	...
	public function actionDownload(){
		if (isset($_GET)) {
			$name = $_GET['nombre'];
			$sPathFile = Yii::getPathOfAlias('webroot').'/archivos/';
			$sDocumentoPath = $sPathFile.$_GET['objeto'].'/'.$_GET['nombre'];
			if (file_exists($sDocumentoPath)) {
				$filecontent=file_get_contents($sDocumentoPath);
